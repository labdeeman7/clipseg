...
def validate(model, dataset, config): #😉the validate loop is here. I also think I should save my validation, then write a test script from now on. Save metrics like the IoU, thhe correct classes etc in my validation. 
    ...
    with torch.no_grad(): #😉 No grad, not training.  This is the validation script.
        ...
        for data_x, data_y in data_loader: #😉 Data loader output. It is just an image and text. 
            ...
            prompts = model.sample_prompts(data_x[1], prompt_list=('a photo of a {}',)) #😉 First get the prompt of the image, I guess data_x, is a list containing the information and the corresponding image.  then make it a photo of {}
...
def main():
    ...
    #2 wandb
    experiment = wandb.init(project='clipseg_endovis2017', resume='allow', anonymous='must')
    ...
    data_loader = DataLoader(dataset, batch_size=batch_size, num_workers=1) #🛑 remember to change back.
    ...
    for epoch in range(epochs): #😉 Trains Starts
        ...
        for data_x, data_y in data_loader:
            ...
            if config.mix: #😉 A form of data augmentation. 🙋‍♂️ is this phrasecut++? 👌THis is for when we are mixing the text and image conditionals. Not dat augmentation per se but ya, it is close. 
                ...
                # data_x[1] = text label
                print(f"data_x[1]{data_x[1]}")
                prompts = model.sample_prompts(data_x[1]) #😉 remeber data_x[0] is img, data_x[1] is the phrase prompt. Depends on the variable shuffle, shuffle+
                ...
                if dataset.__class__.__name__ == 'PhraseCut': #😉If we are using phrasecut, 
                    ...
                    text_weights = torch.max(text_weights[:,0], 1 - visual_is_valid.float().cuda()).unsqueeze(1) #😉 text weghts an either be the current amount, or the 1-visual is valid for phrasecut dataset.
